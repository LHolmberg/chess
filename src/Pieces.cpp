...
bool Piece::CheckPieceAt(int x, int y, std::array<Piece *, 32> pieces)
{
    for (int i = 0; i < pieces.size(); i++)
    {
        ...
        {
            if ((pieces[i]->team != this->team) && ((this->team == "WHITE" && this->turn == true) ||
                                                    this->team == "BLACK" && this->turn == false))
            {
                pieces[i]->square.x = H * 2;
                pieces[i]->square.y = H * 2;
                if (pieces[i]->name() == "Queen")
                {
                    std::string winnerTxt = pieces[i]->team == "WHITE" ? "Black won!" : "White won!";
                    std::cout << "\n\n\n###########\n >> " + winnerTxt + "\n###########\n\n\n";
                    throw std::exception();
                }
                return true;
