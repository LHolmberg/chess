...
bool Pawn::PossibleMove(std::pair<int, int> mapgrid[8][8], std::pair<int, int> newPos, std::array<Piece *, 32> pieces)
{
    ...
    if (this->team == "BLACK")
    {
        if ((newPos.second == mapgrid[n - 1][k].second || (newPos.second == mapgrid[n - 2][k].second && y == 450)) && newPos.first == mapgrid[n][k].first)
        {
            std::cout << y << std::endl;
    ...
    {
        if ((newPos.second == mapgrid[n + 1][k].second || (newPos.second == mapgrid[n + 2][k].second && y == 75)) && newPos.first == mapgrid[n][k].first)
        {
            if (CheckPieceAt(newPos.first, newPos.second, pieces) == true)
                return true;
        }
